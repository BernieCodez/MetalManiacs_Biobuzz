...
class HeadingTuner extends TuningOpMode<Double> {
    ...
    private static final double RUNTIME = 2.0;
    ...
    //blabdlasb
    private int diagnosticN = 0;
    private double diagnosticA = 0;
    private double diagnosticK = 0;
    private double diagnosticMinVel = 0;
    private double diagnosticMaxVel = 0;
    private int diagnosticAccepted = 0;
    private int diagnosticRejectedLow = 0;
    private int diagnosticRejectedHigh = 0;
    private double diagnosticSlope = Double.NaN;
    private double diagnosticIntercept = Double.NaN;
    ...
    private double calculatekP(double alpha) {
        double kP = tau * alpha * alpha / K;

        telemetry.addLine("=== HEADING SYSTEM ID ===");
        telemetry.addData("Min normalized velocity", diagnosticMinVel);
        telemetry.addData("Max normalized velocity", diagnosticMaxVel);

        telemetry.addData("Accepted", diagnosticAccepted);
        telemetry.addData("Rejected low", diagnosticRejectedLow);
        telemetry.addData("Rejected high", diagnosticRejectedHigh);

        telemetry.addData("10% K", 0.1 * diagnosticK);
        telemetry.addData("80% K", 0.8 * diagnosticK);

        telemetry.addData("N", diagnosticN);
        telemetry.addData("A", diagnosticA);
        telemetry.addData("K", diagnosticK);

        telemetry.addData("Slope", diagnosticSlope);
        telemetry.addData("Intercept", diagnosticIntercept);

        telemetry.addData("tau", tau);
    ...
    private void systemIdentification() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            double logValue = Math.log(K - vel);

            x.add(times.get(i));
            y.add(logValue);
        }

        diagnosticMinVel = minVel;
        diagnosticMaxVel = maxVel;
        diagnosticAccepted = x.size();
        diagnosticRejectedLow = rejectedLow;
        diagnosticRejectedHigh = rejectedHigh;

        diagnosticSlope = Double.NaN;
        diagnosticIntercept = Double.NaN;
        ...
        diagnosticSlope = linReg[1];
        diagnosticIntercept = linReg[0];

        if (!Double.isFinite(linReg[1])) {
            tau = Double.NaN;
            return;
        }
...
class ForwardBraking extends TuningOpMode<List<Double>> {
    ...
    protected List<Double> runTuningOpMode() throws InterruptedException {
        ...
        while (state != State.DONE && !isStopRequested()) {
            ...
            telemetry.addData("VX", localizer.velocity().vx);
            ...
            switch (state) {
                ...
                case DRIVE: {
                    ...
                    boolean reachedTarget =
                            (direction > 0 && localizer.pose().x() >= distance)
                                    || (direction < 0 && localizer.pose().x() <= 12);
                    ...
                    if (reachedTarget) {
                        ...
                        telemetry.addLine(
                                "!!!!!!!! BRAKE CONDITION TRIGGERED !!!!!!!!"
                        );

                        startPosition = localizer.pose().toVector2D();
                        measuredVelocity =
                                localizer.velocity().toVector2D().magnitude();
    ...
    private double getHeadingPower(Localizer localizer) {
        ...
        telemetry.addData("headingLinear", headingLinear);
        telemetry.addData("headingQuadratic", headingQuadratic);
        telemetry.addData("headingKP", headingKP);
        telemetry.addData("angularVel", angularVel);
        telemetry.addData("currentHeading", currentHeading);

        telemetry.addData("headingLinear NaN", Double.isNaN(headingLinear));
        telemetry.addData("headingQuadratic NaN", Double.isNaN(headingQuadratic));
        telemetry.addData("headingKP NaN", Double.isNaN(headingKP));
        telemetry.addData("angularVel NaN", Double.isNaN(angularVel));
        telemetry.addData("heading NaN", Double.isNaN(currentHeading));

        double brakeDist =
                headingLinear * angularVel +
                        headingQuadratic * angularVel * angularVel * Math.signum(angularVel);

        double headingError = Angle.normalizeSigned(-currentHeading);
        double error = headingError - brakeDist;

        double result = Utils.clamp(headingKP * error, -0.3, 1.0) / 2;

        telemetry.addData("brakeDist", brakeDist);
        telemetry.addData("headingError", headingError);
        telemetry.addData("headingError NaN", Double.isNaN(headingError));
    ...
    private void brake(Drivetrain drivetrain, Localizer localizer) {
        ...
        telemetry.addData("Brake Power", brake);
        telemetry.addData("Heading Power", headingPower);
        telemetry.addData("VX During Brake", localizer.velocity().vx);
        telemetry.addData("Brake Timer", timer.seconds());
        telemetry.update();
